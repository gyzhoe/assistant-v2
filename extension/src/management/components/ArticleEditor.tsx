...
export function ArticleEditor({ onBack, mode = 'create', articleId }: ArticleEditorProps): React.ReactElement {
  ...
  const mutation = useMutation<CreateArticleResponse | UpdateArticleResponse, Error>({
    ...
    onError: (err: Error) => {
      ...
      } else if (err instanceof ApiError && (err.body as Record<string, unknown>)?.['error_code'] === 'LLM_DOWN') {
        setError('Embedding service is unavailable. Is the LLM server running?')
      } else {
      ...
      <div className="editor-fields">
        ...
        <div className="editor-field">
          ...
          <TagsSection
            ...
            tagTruncateWarning={tagTruncateWarning}
            setTagTruncateWarning={setTagTruncateWarning}
          />
        </div>
